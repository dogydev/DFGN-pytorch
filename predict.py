from DFGN.pytorch_pretrained_bert.modeling import BertModel
...
from DFGN.tools.data_iterator_pack import DataIteratorPack
from DFGN.text_to_tok_pack import *
from create_dev_graph import iter_data
...
if __name__ == "__main__":
    args = set_config()
    os.makedirs('DFGN/ckpt', exist_ok=True)
    # os.makedirs('output', exist_ok=True)
    setting_file = 'DFGN/ckpt/run_settings.json'
    checkpoint_path = 'DFGN/ckpt/checkpoint.pth'
    prediction_path = 'output.json'
    ...
    with gzip.open('work_dir/dev/examples.pkl.gz', 'rb') as fin:
        examples = pickle.load(fin)
        example_dict = {e.qas_id: e for e in examples}

    with gzip.open('work_dir/dev/features.pkl.gz', 'rb') as fin:
        features = pickle.load(fin)
        feature_dict = {f.qas_id: f for f in features}

    graph_dict = iter_data("work_dir/dev/examples.pkl.gz", "work_dir/dev/features.pkl.gz", "work_dir/dev"
                                                                                           "/query_entities.json")
